...
module.exports = {
   ...
   * @example setCheckUpdates(true);
   * @param bool - True to check for updates, false to not check for updates.<boolean>
   * @type {boolean} bool
  */
  setCheckUpdates(bool) {
    if (bool === true) {
  ...
  moveToMongo(jsondb) {
    console.log('Mzrdb json to mzrdb mongodb: Started copying database.');

    Object.keys(jsondb).map(async (data) => {
      await this.adapter.set(data, jsondb[data]);

      console.log(`Mzrdb json to mzrdb mongodb: Copied ${data}`);
    });
