...
const kişi = ['guabey', 'rhtrock', 'Streamer', 'Yazılımıcı', 'Grafik Tasarımcısı', 'Editör'];
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (sonDilenme !== null && süre - (Date.now() - sonDilenme) > 0) {
            ...
            if (randomKişi === 'guabey') {
                dilen = mzrdjs.random(10, 15);
                mzrdb.add(`mzrbakiye.${user.id}`, dilen);
                title = "Whattt guabey Sana Para Verdi!!!"
                description = `Üzülme ben geldim seni kurtarmaya al bu **${dilen} BL**'yi gününü iyi geçir :)`;
                color = 'Green';
            } else {
                dilen = mzrdjs.random(5, 10);
                mzrdb.add(`mzrbakiye.${user.id}`, dilen);
                title = 'Güzel Bir Adam Sana Para Verdi :)'
                description = `Senin yerinde bizde olabilirdik kardeşim **${dilen} BL** varmış onu veriyorum.`;
                color = 'Blue';
            }
